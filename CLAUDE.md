...
| 12 | Accumulator xMult | `SIGIL_ACCUMULATOR_XMULT` | `getAccumulatorXMult(sigils, accumulators)` + `applyAccumulatorIncrements(sigils, accumulators, eventCounts)` | Executioner (`{ trigger: "criticalHit", perEventDelta: 0.2, initialValue: 1.0 }` — gains +0.2x Mult per Critical Hit). xMult factor is read from `player.sigilAccumulators` (MapSchema<string, number>) keyed by sigil ID. **Timing**: the pre-cast accumulator value applies to the current cast as an xMult; the server increments AFTER cast resolution via `applyAccumulatorIncrements` using per-trigger event counts (today just `criticalHit`). Persists across rounds within a run, resets on new run. SigilBar tooltip surfaces the live xMult so the player can track build-up. |
