...
1. `castSpell()` in `arkynAnimations.ts` captures DOM positions for flight tweens, then delegates the damage/bubble/event calculation to `assembleCastBreakdown()` in `client/animations/castBreakdown.ts`.
2. `assembleCastBreakdown()` is the pure-ish helper that: resolves the spell, calls `composeCastModifiers()` (see Damage Model), runs `calculateSpellDamage` with identical inputs to the server, iterates sigil procs via `iterateProcs`, and emits `{ runeBreakdown, bubbles, procBubblesForCast, handMultBubblesForCast, totalDamage, spellBaseDamage, baseTotal, hasCritical, hasAnyProc, hasAnyMultEvent, ... }`. The wrapper orchestrates 5 phase sub-functions in the same file — `iterateCastProcs` (proc iteration + execute clamp + baseTotal adjustment), `buildRuneDamageEvents` (per-rune loop with interleaved procs / played-mult / element-rune-bonus / accumulator-inc events), `buildHandMultEvents` (Synapse-style), `buildFlatMultEvents` (inventory + cast-rng + spell-tier mult), `buildXMultEvents` (static + accumulator xMult reveal). Each sub-function owns one phase's event assembly and threads `eventIdx` through for stagger timing. `castSpell` stays lean orchestration (flight capture → breakdown call → timeline callback wiring).
...
Sigils are shop-purchased items that modify a run. **18 sigils are implemented today** across **12 effect categories**, and the system is designed to scale to 50+ via **data-driven effect registries** — adding most new sigils is a single data entry, zero code branches.
...
- `shared/sigilEffects.ts` — category-based registries for mechanics, plus pure helpers consumers call generically. Every registry-backed helper uses the shared `forEachOwnedSigil(sigils, registry, fn)` primitive, which handles Mimic expansion + null-check in one place — new category helpers should call it instead of hand-rolling the iteration shell. (The exceptions are `iterateProcs`, which is a `Generator`, and `applyAccumulatorIncrements`, which intentionally iterates the RAW sigils list so Mimic doesn't double-update accumulator storage keyed by sigil id.)
...
| 2 | RNG procs on played runes | `SIGIL_PROCS` | `iterateProcs(sigils, elements, seed, round, cast, isCritical?)` | Voltage (25% on lightning → double_damage), Fortune (33% on critical → +2 gold), Hourglass (25% any element → double_damage), Keychain (100% on first critical rune → double_damage, retriggerCount 2 = 2 extra hits). `ProcDefinition` supports `firstMatchOnly` (stop after first matching rune — Keychain pattern) and `retriggerCount` (number of yields per successful roll, default 1 — Keychain yields 2). |
...
| 10 | Element-rune bonus | `SIGIL_ELEMENT_RUNE_BONUS` | `getElementRuneBonus(sigils, elements, isCritical)` | Engine (`{ elements: ["steel"], baseBonus: 4, multBonus: 2 }` — +4 Base post-modifier and +2 Mult per Steel rune), Lex Divina (`{ elements: ["holy"], requireCritical: true, baseBonus: 8, multBonus: 2 }` — crit-gated variant), Alkahest (`{ elements: ["poison", "arcane"], baseBonus: 10, multBonus: 4 }` — multi-element filter). Optional `requireCritical: true` gates the bonus on weakness hits; omit `elements` to trigger on any rune. |
| 11 | Scroll level bonus | `SIGIL_SCROLL_LEVEL_BONUS` | `getScrollLevelsPerUse(sigils)` | Scroll God (`1` — grants +2 levels per scroll instead of +1). Additive across owned sigils; the shop handler, consumable handler, and upgrade animation all call the helper so server and client agree on how many levels a scroll grants. Shop upgrade animation loops the shake + display + hold cycle once per level step, so +2 scrolls read as "+0 → +2" then "+2 → +4" in sequence. |
| 12 | Accumulator xMult | `SIGIL_ACCUMULATOR_XMULT` | `getAccumulatorXMult(sigils, accumulators)` + `applyAccumulatorIncrements(sigils, accumulators, eventCounts)` | Executioner (`{ trigger: "criticalHit", perEventDelta: 0.1, initialValue: 1.0 }` — gains +0.1x Mult per Critical Hit). xMult factor is read from `player.sigilAccumulators` (MapSchema<string, number>) keyed by sigil ID. **Timing**: the pre-cast accumulator value applies to the current cast as an xMult; the server increments AFTER cast resolution via `applyAccumulatorIncrements` using per-trigger event counts (today just `criticalHit`). Persists across rounds within a run, resets on new run. SigilBar tooltip surfaces the live xMult so the player can track build-up. |

**Additive vs multiplicative mult**: Categories 3 (hand-mult), 9 (played-mult), and 10 (element-rune-bonus mult portion) all feed the additive `bonusMult` channel; their values sum. Category 6 (spell xMult) is multiplicative and applies AFTER the additive sum: `finalMult = (tierMult + bonusMult) × xMult`. Proc damage from Voltage/Hourglass uses the same final mult, so additive + multiplicative sigils stack cleanly with procs.

**Post-modifier per-rune base bonus** (Category 10): The base portion of `SIGIL_ELEMENT_RUNE_BONUS` is distinct from mult — it's a FLAT per-rune base added AFTER the resist/weak ×-modifier has been applied (so "+4 Base" reads as "+4 to the number that lands", not "+4 that then gets doubled by weakness"). `composeCastModifiers` returns this as `perRuneBaseBonus: number[]`, and `calculateSpellDamage` accepts it as an optional parameter that gets added directly to each rune's post-modifier contribution. Because the bonus lands inside `runeBaseContributions[i]`, the per-rune damage bubble, proc double-damage re-pops, and `baseTotal` automatically pick it up.
