...
| 10 | Element-rune bonus | `SIGIL_ELEMENT_RUNE_BONUS` | `getElementRuneBonus(sigils, elements, isCritical)` | Engine (`{ element: "steel", baseBonus: 4, multBonus: 2 }` — +4 Base post-modifier and +2 Mult per Steel rune), Lex Divina (`{ element: "holy", requireCritical: true, baseBonus: 8, multBonus: 2 }` — crit-gated variant). Optional `requireCritical: true` gates the bonus on weakness hits; omit for flat per-element bonus. |
