...
export interface AcquiredRuneStats {
    ...
    totalBase: number;
    /** Starting pouch + all acquired (ignores banished — used for the
     *  modal's pouch/hand/spent accounting where banished reads as spent). */
    totalAll: number;
...
export function useAcquiredRuneStats(): AcquiredRuneStats {
    ...
    return {
        ...
        totalAll: POUCH_SIZE + acquiredRunes.length,
